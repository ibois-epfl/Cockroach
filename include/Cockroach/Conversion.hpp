...
namespace Cockroach
{
	...
	inline void convert_CilantroToOpen3DCloud(std::shared_ptr<cilantro::PointCloud3f> cilantro_cloud_f, std::shared_ptr<PC> open3d_cloud_d)
	{
	...
	inline void convert_CilantroToOpen3DCloud_multiple(std::vector<std::shared_ptr<cilantro::PointCloud3f>> cilantro_clouds_f, std::vector<std::shared_ptr<PC>>& open3d_clouds_d) {
		open3d_clouds_d.reserve(cilantro_clouds_f.size());

		std::cout << "convert_CilantroToOpen3DCloud_multiple" << std::endl << std::endl;
		for (auto& cilantro_cloud : cilantro_clouds_f) {
			std::shared_ptr<open3d::geometry::PointCloud> cloud_op3(new  open3d::geometry::PointCloud);
			convert_CilantroToOpen3DCloud(cilantro_cloud, cloud_op3);
			open3d_clouds_d.emplace_back(cloud_op3);
		}
	...
	inline void convert_Open3DToCilantroCloud(std::shared_ptr<PC> open3d_cloud_d, std::shared_ptr<cilantro::PointCloud3f> cilantro_cloud_f)
	{
		// Convert points
		auto ptt = cilantro_cloud_f->points;
