...
namespace Cockroach
{
	...
	inline  RS evaluateRegistration(std::shared_ptr<PC> cloudSource,
	...
	inline  RS ICPPointToPoint(std::shared_ptr<PC> cloudSource,
		std::shared_ptr<PC> cloudTarget,
	...
	inline  RS ICPPointToPlane(std::shared_ptr<PC> cloudSource,
		std::shared_ptr<PC> cloudTarget,
		Eigen::Matrix4d_u initTrans,
		float threshold,
	...
	/// \param rel_RMSE Boundary of RMSE (the precision of the registration).
	/// \param close_points Number of points to be estimate the normals(the more the better).
	inline  RS ICPColored(std::shared_ptr<PC> cloudSource,
		std::shared_ptr<PC> cloudTarget,
		Eigen::Matrix4d_u initTrans,
		float threshold,
		bool enable_robust_kernel = false,
	...
	inline  RS registrationRANSAC(std::shared_ptr<PC> cloudSource,
		std::shared_ptr<PC> cloudTarget,
	...
	inline  RS registrationFast(std::shared_ptr<PC> cloudSource,
		std::shared_ptr<PC> cloudTarget,
		double voxel_size = 0.005,
		...
		double checker_normal_degAngle = 30) // in radians 0.5 ~ 30 deg
	{
		...
		RS reg_res_fast = open3d::pipelines::registration::FastGlobalRegistrationBasedOnFeatureMatching(*low_res_clouds[0],
			*low_res_clouds[1],
			*FPFH_features[0],
			*FPFH_features[1],
			fast_options);
